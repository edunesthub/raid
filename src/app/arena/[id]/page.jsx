    ...
    <div className="min-h-screen bg-black text-white flex flex-col lg:flex-row h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto lg:h-full p-4 lg:p-8 space-y-8 no-scrollbar bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-[#1a1a1d] via-black to-black">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8 relative z-10">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <div className="px-3 py-1 bg-orange-500 rounded-full text-[10px] font-black uppercase tracking-widest animate-pulse">
          ...
          <div className="flex items-center gap-3">
             {challenge.code && (
               <button 
                 onClick={copyCode}
                 className="flex items-center gap-2 px-6 py-4 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition-all group"
               >
                 <span className="text-sm font-black tracking-[0.2em]">{challenge.code}</span>
                 {copied ? <Check size={16} className="text-green-500" /> : <Copy size={16} className="text-orange-500 group-hover:scale-110 transition-transform" />}
               </button>
             )}
             <button className="p-4 bg-orange-500 rounded-2xl shadow-lg shadow-orange-500/20 hover:scale-110 transition-transform active:scale-95">
               <Share2 size={20} />
             </button>
          </div>
        </div>
        ...
        <div className="relative aspect-video w-full max-w-5xl mx-auto bg-[#0f0f10] border border-white/5 rounded-[40px] overflow-hidden group shadow-[0_0_100px_-20px_rgba(0,0,0,0.8)]">
           ...
           <div className="relative z-10 text-center space-y-6 max-w-md mx-auto p-12 bg-black/60 backdrop-blur-3xl rounded-[40px] border border-white/10 shadow-2xl">
             <div className="flex items-center justify-center gap-12">
               <div className="text-center group-hover:scale-110 transition-transform duration-500">
                  <div className="w-24 h-24 rounded-3xl border-4 border-orange-500 p-1 mb-4 shadow-[0_0_30px_rgba(249,115,22,0.4)]">
                    <img src={user?.photoURL || '/assets/avatar-placeholder.png'} className="w-full h-full rounded-2xl object-cover" />
                  </div>
                  <span className="text-xs font-black uppercase text-white">{user?.displayName?.split(' ')[0] || 'Player 1'}</span>
               </div>
               
               <div className="flex flex-col items-center gap-2">
                 <span className="text-3xl font-black italic text-orange-500">VS</span>
                 <Skull size={32} className="text-gray-700 animate-pulse" />
               </div>

               <div className="text-center group-hover:scale-110 transition-transform duration-500 delay-75">
                  <div className="w-24 h-24 rounded-3xl border-4 border-gray-800 p-1 mb-4 bg-gray-900 flex items-center justify-center">
                    <Users size={40} className="text-gray-700" />
                  </div>
                  <span className="text-xs font-black uppercase text-gray-500">Waiting...</span>
               </div>
             </div>
             
             <div className="pt-8">
               <button className="px-12 py-5 bg-white text-black font-black uppercase tracking-widest text-[10px] rounded-full hover:bg-orange-500 hover:text-white transition-all shadow-xl shadow-white/5">
                 READY UP
               </button>
             </div>
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
          <div className="bg-[#0f0f10] border border-white/5 rounded-3xl p-6 hover:border-orange-500/20 transition-all flex flex-col gap-2">
            <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Rounds</span>
            <span className="text-2xl font-black text-white italic">BEST OF {challenge.rounds}</span>
          </div>
          <div className="bg-[#0f0f10] border border-white/5 rounded-3xl p-6 hover:border-orange-500/20 transition-all flex flex-col gap-2">
            <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Prize Type</span>
            <span className="text-2xl font-black text-orange-500 italic">RANKED XP <Zap size={20} className="inline ml-1" fill="currentColor" /></span>
          </div>
          <div className="bg-[#0f0f10] border border-white/5 rounded-3xl p-6 hover:border-orange-500/20 transition-all flex flex-col gap-2">
            <span className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">Visibility</span>
            <span className="text-2xl font-black text-white italic">{challenge.visibility?.toUpperCase()} HUB</span>
          </div>
        </div>
      </div>
