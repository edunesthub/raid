...
export default function ChallengeRoomPage() {
  ...
  useEffect(() => {
    ...
    const setupChallenge = async () => {
      ...
      try {
        ...
        // Subscribe to real-time updates for challenge state
        unsubscribeChallenge = arenaService.subscribeToChallenge(id, (updatedChallenge) => {
          if (updatedChallenge) {
            setChallenge(updatedChallenge);
          }
        });

        // Subscribe to real-time updates for arena chat
        unsubscribeChat = chatService.subscribeToArenaChat(id, (newMessages) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      await chatService.sendArenaMessage(id, {
        senderId: user.uid,
    ...
    <div className="min-h-screen bg-black text-white flex flex-col lg:flex-row h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto lg:h-full p-4 lg:p-8 space-y-8 no-scrollbar bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-[#1a1a1d] via-black to-black">
        ...
        <div className="relative aspect-video w-full max-w-5xl mx-auto bg-[#0f0f10] border border-white/5 rounded-[40px] overflow-hidden group shadow-[0_0_100px_-20px_rgba(0,0,0,0.8)]">
          ...
          <div className="relative z-10 text-center space-y-4 max-w-md mx-auto p-6 md:p-12 bg-black/60 backdrop-blur-3xl rounded-3xl md:rounded-[40px] border border-white/10 shadow-2xl">
            ...
            <div className="pt-4 md:pt-8">
              <button
                ...
                className={`px-8 md:px-12 py-3 md:py-5 font-black uppercase tracking-widest text-[9px] md:text-[10px] rounded-full transition-all shadow-xl ${challenge.status === 'active'
          ...
          {/* Live Label */}
          <div className="absolute top-0 md:top-8 right-4 md:right-8 flex items-center gap-2 bg-black/80 backdrop-blur-xl px-3 md:px-4 py-1.5 md:py-2 rounded-full border border-white/10 z-20">
          ...
          {messages.map(msg => (
            <div key={msg.id} className="flex flex-col gap-1.5 animate-in slide-in-from-bottom-2 duration-300">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className={`text-[11px] font-black italic uppercase tracking-tighter ${msg.senderId === user?.uid ? 'text-white' : 'text-orange-500'}`}>
                  {msg.senderId === user?.uid ? 'You' : msg.senderName}
                </span>
              </div>
              <div className={`text-xs font-medium px-4 py-2.5 rounded-2xl border ${
