// src/app/admin/components/NonBracketResults.jsx
'use client';

import { useState, useEffect } from 'react';
import { Trophy, Users, Award, X, Search, Crown, Medal } from 'lucide-react';
import { 
  collection, 
  query, 
  where, 
...
export default function NonBracketResults() {
  ...
  useEffect(() => {
    loadNonBracketTournaments();
  }, []);

  const loadNonBracketTournaments = async () => {
    try {
      setLoading(true);
      // Get live tournaments without bracket
      const q = query(
      ...
      const tournamentsList = snapshot.docs
        .map(doc => ({ id: doc.id, ...doc.data() }))
        .filter(t => !t.bracketGenerated); // Only non-bracket tournaments
  ...
  const handleSelectWinner = (placement, participantId) => {
    // Prevent selecting same player for multiple placements
  ...
  const submitResults = async () => {
    ...
    if (!confirm('Confirm tournament results? This will complete the tournament and cannot be undone.')) {
      return;
    }
    ...
    try {
      ...
      if (placements.first) {
        updates.push(
          updateDoc(
            doc(db, 'tournament_participants', `${selectedTournament.id}_${placements.first}`),
            { placement: 1, placementAt: serverTimestamp() }
          )
        );
      }
      ...
      if (placements.second) {
        updates.push(
          updateDoc(
            doc(db, 'tournament_participants', `${selectedTournament.id}_${placements.second}`),
            { placement: 2, placementAt: serverTimestamp() }
          )
        );
      }
      ...
      if (placements.third) {
        updates.push(
          updateDoc(
            doc(db, 'tournament_participants', `${selectedTournament.id}_${placements.third}`),
            { placement: 3, placementAt: serverTimestamp() }
          )
        );
      }
  ...
  const getPlacementIcon = (placement) => {
    switch (placement) {
      case 'first':
        return { icon: Crown, color: 'text-yellow-400', bg: 'bg-yellow-500/20', border: 'border-yellow-500/40' };
      case 'second':
        return { icon: Medal, color: 'text-gray-400', bg: 'bg-gray-500/20', border: 'border-gray-500/40' };
      case 'third':
        return { icon: Medal, color: 'text-orange-400', bg: 'bg-orange-500/20', border: 'border-orange-500/40' };
      default:
        return { icon: Award, color: 'text-gray-400', bg: 'bg-gray-500/20', border: 'border-gray-500/40' };
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Select winners for Battle Royale, FFA, and other non-bracket tournaments
          </p>
        </div>
        <button
          onClick={loadNonBracketTournaments}
      ...
      {!loading && tournaments.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-bold text-white mb-2">
            No Non-Bracket Tournaments
          </h3>
          <p className="text-gray-400">
            There are no active Battle Royale or FFA tournaments at the moment
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="sticky top-0 bg-gray-900 border-b border-gray-700 p-6 flex items-center justify-between z-10">
              <div>
                <h3 className="text-2xl font-bold text-white">Select Tournament Winners</h3>
            ...
            <div className="p-6 space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h4 className="font-bold text-white">
                          {placement === 'first' ? '1st' : placement === 'second' ? '2nd' : '3rd'} Place
                        </h4>
                      </div>
                      ...
                      {selected ? (
                        <div className="bg-gray-800 rounded-lg p-3 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-full overflow-hidden bg-gradient-to-r from-orange-600 to-orange-400">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl pl-10 pr-4 py-3 text-white focus:outline-none focus:border-orange-500"
                />
              </div>
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full overflow-hidden bg-gradient-to-r from-orange-600 to-orange-400">
                            ...
                            {!isSelected && (
                              <>
                                <button
                                  ...
                                  disabled={placements.first === participant.id}
                                  className="px-2 py-1 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-400 rounded text-xs font-semibold transition-colors disabled:opacity-50"
                                >
                                  1st
                                </button>
                                <button
                                  onClick={() => handleSelectWinner('second', participant.id)}
                                  disabled={placements.second === participant.id}
                                  className="px-2 py-1 bg-gray-500/20 hover:bg-gray-500/30 text-gray-400 rounded text-xs font-semibold transition-colors disabled:opacity-50"
                                >
                                  2nd
                                </button>
                                <button
                                  onClick={() => handleSelectWinner('third', participant.id)}
                                  disabled={placements.third === participant.id}
                                  className="px-2 py-1 bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 rounded text-xs font-semibold transition-colors disabled:opacity-50"
                                >
                                  3rd
                                </button>
                              </>
