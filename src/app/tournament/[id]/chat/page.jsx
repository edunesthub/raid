...
export default function TournamentChatPage({ params }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col">
                  ...
                  .map((participant) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 p-2.5 rounded-lg transition-colors ${participant.id === user?.id
                            ? 'cursor-default'
                  ...
                  <div className={`flex gap-2 ${isOwn ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`flex-1 min-w-0 ${isOwn ? 'text-right' : ''}`}>
                      ...
                      <div className="group relative inline-block max-w-[96%] sm:max-w-[88%]">
                        <div
                          className={`inline-block px-3 py-2 rounded-2xl text-sm ${isOwn
      ...
      {/* Input Area */}
      <div className="fixed bottom-0 left-0 right-0 bg-black/40 backdrop-blur-md border-t border-gray-800/50 p-3 pb-5 pb-safe z-20">
        <form onSubmit={handleSendMessage} className="flex gap-2 items-center">
          <input
            ...
            className="flex-1 min-w-0 bg-gray-900/50 text-white placeholder-gray-500 rounded-full px-4 py-2.5 text-base focus:outline-none focus:ring-2 focus:ring-orange-500/50 border border-gray-800/50"
          />
          <button
            type="submit"
            disabled={!newMessage.trim()}
            className="w-10 h-10 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-full transition-colors flex-shrink-0 flex items-center justify-center"
          >
